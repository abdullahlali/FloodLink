...
function defineViewProperty(component) {
    ...
    Object.defineProperty(component.el, "view", {
        get: originalAccessor.get.bind(component.el),
        set: (newView) => {
            originalAccessor.set.call(component.el, newView);
...
function areCamerasEqual(camera, camera2) {
    return (camera2 != null &&
...
function parseCenter(newValue, view) {
    return parsePoint(view?.center, newValue);
}
function parsePoint(currentValue, newValue) {
    if (typeof newValue === "string") {
        return parsePoint(currentValue, newValue.split(",").map(Number));
    }
    if (!currentValue) {
        return newValue;
    }
    if (Array.isArray(newValue)) {
        const longitude = newValue[0];
        const latitude = newValue[1];
        const z = newValue.length > 2 ? newValue[2] : undefined;
        if (longitude !== currentValue.longitude || latitude !== currentValue.latitude || z !== currentValue.z) {
            return newValue;
        }
    }
    else if (newValue && !currentValue?.equals(newValue)) {
        return newValue;
    }
    return undefined;
}
...
export { parsePoint as a, areCamerasEqual as b, isNotSameViewpoint as c, attachToParent as d, isNullOrEmptyString as e, defineViewProperty as f, addHandles as g, isNotSameEnvironment as i, parseCenter as p, referenceElementHelper as r };
